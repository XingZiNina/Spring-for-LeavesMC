...
public class InternalBytebufHandler {
    ...
    public static void init() {
        ChannelInitializeListenerHolder.addListener(Key.key("leaves:bytebuf"), channel -> {
            ...
            if (LeavesConfig.mics.packetProtector) {
                channel.pipeline().addBefore("prepender", PacketProtector.handlerName, new PacketProtector());
            }
            channel.pipeline().addBefore("splitter", NetworkStatsHandler.downloadHandlerName, new DownloadStatsHandler());
            channel.pipeline().addBefore("prepender", NetworkStatsHandler.uploadHandlerName, new UploadStatsHandler());
        });
    }
    ...
    private static class PacketHandler extends ChannelDuplexHandler {
        ...
        public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
            if (msg instanceof ClientboundLevelChunkWithLightPacket chunkPacket && LeavesConfig.mics.packetProtector) {
                PacketProtector protector = (PacketProtector) ctx.pipeline().get(PacketProtector.handlerName);
                if (protector != null) {
                    protector.trackChunk(chunkPacket.getX(), chunkPacket.getZ());
                }
            }
    ...
    private abstract static class NetworkStatsHandler extends ChannelDuplexHandler {

        private final static String downloadHandlerName = "leaves-network-download-stats-handler";
        private final static String uploadHandlerName = "leaves-network-upload-stats-handler";
    }

    private static class DownloadStatsHandler extends NetworkStatsHandler {

        @Override
        public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
            if (LeavesConfig.mics.networkSpeed && msg instanceof ByteBuf buf) {
                NetworkSpeedMonitor.recordDownload(buf.readableBytes());
            }
            super.channelRead(ctx, msg);
        }
    }

    private static class UploadStatsHandler extends NetworkStatsHandler {

        @Override
        public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
            if (LeavesConfig.mics.networkSpeed && msg instanceof ByteBuf buf) {
                NetworkSpeedMonitor.recordUpload(buf.readableBytes());
            }
            super.write(ctx, msg, promise);
        }
    }
