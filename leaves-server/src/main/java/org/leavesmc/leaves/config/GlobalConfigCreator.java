...
public class GlobalConfigCreator {
    ...
    private static void initCategory(@NotNull Field categoryField, @NotNull GlobalConfigCategory globalCategory, @Nullable Object upstreamField, @NotNull String upstreamPath) {
        try {
            ...
            if (!globalCategory.comment().isEmpty()) {
                config.setComments(categoryPath.substring(0, categoryPath.length() - 1), Collections.singletonList(globalCategory.comment()));
            }
    ...
    private static void initConfig(@NotNull Field field, GlobalConfig globalConfig, @Nullable Object upstreamField, @NotNull String upstreamPath) {
        try {
            ...
            config.set(path, verifiedConfig.validator().saveConvert(field.get(upstreamField)));
            if (!globalConfig.comment().isEmpty()) {
                config.setComments(path, Collections.singletonList(globalConfig.comment()));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
